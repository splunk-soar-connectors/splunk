...
class SplunkConnector(BaseConnector):
        ...
        try:
            ...
            if error_message == consts.SPLUNK_ERR_MESSAGE_UNAVAILABLE:
                ...
                error_message = "{} ({})".format(
                    error_message, sys.exc_info()[-1].tb_lineno
                )
            ...
            if not error_code:
                ...
                error_message = "Error Code: {}. Error Message: {}".format(
    ...
    def initialize(self):
        ...
        self._base_url = "https://{0}:{1}/".format(
            self.splunk_server, config.get(phantom.APP_JSON_PORT, 8089)
        )
    ...
    def _connect(self, action_result):
        ...
        try:
            ...
            if "405 Method Not Allowed" in error_text:
                ...
                return action_result.set_status(
                    phantom.APP_ERROR,
                    "Error occurred while connecting to the Splunk server. Details: {}".format(
                        error_text
                    ),
                )
        except Exception as e:
        ...
        if parameter is not None:
            ...
            if not allow_zero and parameter == 0:
                return (
                    action_result.set_status(
                        phantom.APP_ERROR,
                        consts.SPLUNK_ERR_INVALID_PARAM.format(param=key),
                    ),
                    None,
                )
    ...
    def _process_json_response(self, r, action_result):
        ...
        if isinstance(resp_json, str):
            ...
            message = "Error from server. Status Code: {}. Error: {}. Error Details: {}".format(
                status_code, error, error_details
            )
            ...
            if resp_json["messages"]:
                ...
                if error_type or error_message:
                    error = "ErrorType: {} ErrorMessage: {}".format(
    ...
    def _resolve_event_id(self, sidandrid, action_result, kwargs_create=dict()):
        ...
        self.send_progress(
            "Running search_query: {}".format(consts.SPLUNK_RID_SID_NOTABLE_QUERY)
        )
    ...
    def _return_first_row_from_query(
        ...
        for search_attempt_count in range(0, RETRY_LIMIT):
            ...
            while True:
                ...
                status = (
                    "Progress: %(progress)03.1f%%   %(scan_count)d scanned   "
    ...
    def _set_splunk_status_dict(self, action_result):
        ...
        for data in entry:
            ...
            is_notable_status_type = (
                data.get("content", {}).get("status_type") == "notable"
            )
            if (
                status_id
                and status_id.isdigit()
                and status_name
                and is_enabled
                and is_notable_status_type
            ):
    ...
    def _update_event(self, param):
        ...
        if wait_for_confirmation:
            ...
            if phantom.is_fail(ret_val):
                return action_result.set_status(
                    phantom.APP_ERROR,
                    "Error occurred while validating the provided event ID. Error: {0}".format(
    ...
    def _get_host_events(self, param):
        ...
        search_query = 'search host="{0}"{1}'.format(
            ip_hostname, " earliest=-{0}d".format(last_n_days) if last_n_days else ""
        )
    ...
    def _on_poll(self, param):  # noqa: C901
        ...
        try:
            if not search_command:
                ...
                search_query = "{0} {1}".format(
                    search_command.strip(), search_string.strip()
                )
        except Exception:
            return action_result.set_status(
        ...
        for item in data:
            ...
            if phantom.is_fail(create_artifact_status):
                self.save_progress(
                    "Error saving artifact: {}".format(create_artifact_msg)
                )
                self.debug_print(
                    "Error saving artifact: {}".format(create_artifact_msg)
                )
    ...
    def _get_tz_str_from_epoch(self, time_format_str, epoch_milli):
        ...
        utc_dt = datetime.utcfromtimestamp(old_div(epoch_milli / 1000)).replace(
            tzinfo=pytz.utc
        )
    ...
    def _test_asset_connectivity(self, param):
        ...
        self.save_progress(
            "Detected Splunk {0}server version {1}".format(
    ...
    def _run_query(
        ...
        for result in results:
            ...
            if (result_index % ten_percent) == 0:
                status = "Finished parsing {0:.1%} of results".format(
                    (float(result_index) / float(result_count))
                )
                self.send_progress(status)
    ...
    def add_json_result(self, action_result):
        ...
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            msg = "Error occurred while adding file to Vault. Error Details: {}".format(
                error_message
            )
